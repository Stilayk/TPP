...
def _pwd_context() -> "CryptContext":
    ...
    # Use a pure-Python scheme to avoid bcrypt backend issues on some platforms.
...
class User(Base):
    ...
    duty_assignments: Mapped[list["DutyAssignment"]] = relationship(back_populates="support_user", cascade="all,delete")
...
def report_to_out(db, report: DailyReport) -> DailyReportOut:
    ...
    return DailyReportOut(
        ...
        employee=UserOut(id=employee.id, username=employee.username, full_name=employee.full_name, role=employee.role),
...
def bootstrap_admin_if_needed(db) -> None:
    ...
    if not settings.BOOTSTRAP_ADMIN_USERNAME or not settings.BOOTSTRAP_ADMIN_PASSWORD or not settings.BOOTSTRAP_ADMIN_FULLNAME:
        raise RuntimeError(
            "Bootstrap admin env vars are required: BOOTSTRAP_ADMIN_USERNAME/PASSWORD/FULLNAME"
        )
...
def admin_list_users(current_user: User = Depends(require_admin), db=Depends(get_db)) -> list[UserOut]:
    ...
    return [UserOut(id=u.id, username=u.username, full_name=u.full_name, role=u.role) for u in users]
...
def get_duties(
    ...
    _ = current_user  # role check only
    assignments = db.execute(
        select(DutyAssignment, User).join(User, DutyAssignment.support_user_id == User.id).where(DutyAssignment.date == date_)
    ).all()

    by_slot: dict[int, User] = {
        assignment.slot: user
...
def generate_duties(payload: DutiesGenerateRequest, current_user: User = Depends(require_admin), db=Depends(get_db)) -> DutiesGenerateOut:
    ...
    with db_session() as tx_db:
        ...
        # Recompute counts inside transaction to be consistent.
        for uid in user_ids:
            counts[uid] = int(
                tx_db.execute(
                    select(func.count())
...
def create_or_get_report(payload: CreateOrGetReportRequest, current_user: User = Depends(get_current_user), db=Depends(get_db)) -> DailyReportOut:
    if current_user.role == "support":
        ...
        # admin
        target_support_id = current_user.id if payload.employee_id is None else payload.employee_id
...
def list_reports(
    ...
    if current_user.role == "support":
        ...
        if employee_id is not None:
            employee_id = ensure_support_user(db, employee_id).id
...
def update_report(
    ...
    if current_user.role == "admin":
        ...
        # support can only update their own
        new_date = report.date
        new_support_id = report.support_user_id

    # uniqueness check if date/support_user_id changed
...
def finalize_report(report_id: int, current_user: User = Depends(get_current_user), db=Depends(get_db)) -> ReportFinalizeOut:
    ...
    if len(report.entries) == 0:
        raise HTTPException(status_code=400, detail="Report has no entries")
...
# If frontend/ directory exists (next to backend/), serve it directly from FastAPI.
# Add this mount AFTER API routes so it doesn't shadow /api/* paths.
